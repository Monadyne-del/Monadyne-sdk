...
exports.detectAccountConflicts = detectAccountConflicts;
exports.validateSchedule = validateSchedule;
const util_1 = require("./util");
function validateInstruction(instr) {
    const issues = [];
    const warnings = [];
    if (!instr.id || typeof instr.id !== 'string')
        issues.push('Invalid or missing id');
    if (!instr.programId || typeof instr.programId !== 'string' || !(0, util_1.isValidPubkey)(instr.programId)) {
    ...
    if (!Array.isArray(instr.accounts) || instr.accounts.length === 0) {
        ...
        instr.accounts.forEach((a, idx) => {
            if (!a || typeof a.pubkey !== 'string' || !(0, util_1.isValidPubkey)(a.pubkey))
                issues.push(`account[${idx}].pubkey invalid: ${String(a && a.pubkey)}`);
            if (typeof a.writable !== 'boolean')
                issues.push(`account[${idx}].writable must be boolean`);
        });
    }
    return { ok: issues.length === 0, issues, warnings };
}
function detectDuplicateIds(schedule) {
    const seen = new Map();
    const dupes = [];
...
function detectAccountConflicts(schedule) {
    ...
    for (const [pubkey, ids] of writableMap.entries()) {
        if (ids.length > 1)
            conflicts.push(`writable conflict on ${pubkey}: ${ids.join(',')}`);
    }
    return conflicts;
}
